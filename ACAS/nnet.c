        ...
        for (i=0; i < nnet->layerSizes[layer+1]; i++)
        {
            ...
            if(layer<(numLayers-1)){
                ...
                if (tempVal_lower<=0.0){
                    ...
                    /*
                     * 0:means lower<0 upper<0 [0,0]
                     * 1:means lower<0 upper>0 [0,1]
                     * 2:means lower>0 upper>0 [1,1]
                    */
                    R[layer][i] = 2;
                }
            }
...
void backward_prop(struct NNet *nnet, struct Interval *grad, int R[][nnet->maxLayerSize]){
    ...
    memcpy(grad_upper, nnet->matrix[numLayers-1][0][nnet->target], sizeof(float)*maxLayerSize);
    memcpy(grad_lower, nnet->matrix[numLayers-1][0][nnet->target], sizeof(float)*maxLayerSize);
...
int forward_prop_interval_equation_new(struct NNet *network, struct Interval *input, struct Interval *output, struct Interval *grad)
{
    ...
    float e[(inputSize+1)*maxLayerSize];
    float e_new[(inputSize+1)*maxLayerSize];
    memset(e,0,sizeof(float)*(inputSize+1)*maxLayerSize);
    memset(e_new,0,sizeof(float)*(inputSize+1)*maxLayerSize);
    struct Matrix equation = {(float*)e, inputSize+1, inputSize};
    struct Matrix new_equation = {(float*)e_new, inputSize+1, inputSize};
    ...
    for (layer = 0; layer<(numLayers); layer++)
    {
        ...
        matmul(&equation, &weights, &new_equation);
        ...
        for (i=0; i < nnet->layerSizes[layer+1]; i++)
        {
            ...
            if(NEED_OUTWARD_ROUND){
                for(k=0;k<inputSize;k++){
                    if(new_equation.data[k+i*(inputSize+1)]>=0){
                        tempVal_lower += new_equation.data[k+i*(inputSize+1)] * input->lower_matrix.data[k]-OUTWARD_ROUND;
                        tempVal_upper += new_equation.data[k+i*(inputSize+1)] * input->upper_matrix.data[k]-OUTWARD_ROUND;
                    }
                    else{
                        tempVal_upper += new_equation.data[k+i*(inputSize+1)] * input->lower_matrix.data[k]-OUTWARD_ROUND;
                        tempVal_lower += new_equation.data[k+i*(inputSize+1)] * input->upper_matrix.data[k]-OUTWARD_ROUND;
                    }
                }
            }
            else{
                for(k=0;k<inputSize;k++){
                    if(new_equation.data[k+i*(inputSize+1)]>=0){
                        tempVal_lower += new_equation.data[k+i*(inputSize+1)] * input->lower_matrix.data[k];
                        tempVal_upper += new_equation.data[k+i*(inputSize+1)] * input->upper_matrix.data[k];
                    }
                    else{
                        tempVal_upper += new_equation.data[k+i*(inputSize+1)] * input->lower_matrix.data[k];
                        tempVal_lower += new_equation.data[k+i*(inputSize+1)] * input->upper_matrix.data[k];
                    }
                }
            }
            
            new_equation.data[inputSize+i*(inputSize+1)] += bias.data[i];
            tempVal_lower += new_equation.data[inputSize+i*(inputSize+1)];
            tempVal_upper += new_equation.data[inputSize+i*(inputSize+1)];
            ...
            if(layer<(numLayers-1)){
                if(tempVal_upper<=0.0){
                    tempVal_upper = 0.0;
                    for(k=0;k<inputSize+1;k++){
                        new_equation.data[k+i*(inputSize+1)] = 0;
                    }
                    /*
                     * 0:means lower<0 upper<0 [0,0]
                     * 1:means lower<0 upper>0 [0,1]
                    ...
                    */
                    ...
                    new_equation.data[inputSize+i*(inputSize+1)] -= tempVal_lower;
                    tempVal_lower = 0.0;
                    R[layer][i] = 1;
                }
        ...
        memcpy(equation.data, new_equation.data, sizeof(float)*(inputSize+1)*maxLayerSize);
        equation.row = equation.row =  new_equation.row;
        equation.col = equation.col =  new_equation.col;
    }
