                     ...
                     int feature_range_length, int split_feature)
{
    ...
    //forward_prop_interval_equation_new2(nnet, input, output, grad);
    ...
    //printf("%f, %f\n", total_avg_depth,avg_depth);
        ...
        if((int)(time_spent)%20==0 && (time_spent - time_spent_last)>10){
            printf("progress: %f%%  total_avg_depth: %f  max_depth:%f avg_depth:%f\n", time_spent/pow(2, total_avg_depth+1)/0.000025/4*100, total_avg_depth, max_depth, avg_depth);
...
int split_interval(struct NNet *nnet, struct Interval *input, struct Interval *output,
                     struct Interval *grad, int depth, int *feature_range,
                     int feature_range_length, int split_feature){
    ...
    struct Interval input_interval1 = {(struct Matrix){input_lower1, 1, nnet->inputSize},
                                        (struct Matrix){input_upper1, 1, nnet->inputSize}};
    struct Interval input_interval2 = {(struct Matrix){input_lower2, 1, nnet->inputSize}, 
                                        (struct Matrix){input_upper2, 1, nnet->inputSize}};

    float o_upper1[nnet->outputSize], o_lower1[nnet->outputSize];
    struct Interval output_interval1 = {(struct Matrix){o_lower1, nnet->outputSize, 1},
                                       (struct Matrix){o_upper1, nnet->outputSize, 1}};
    float o_upper2[nnet->outputSize], o_lower2[nnet->outputSize];
    struct Interval output_interval2 = {(struct Matrix){o_lower2, nnet->outputSize, 1},
                                       (struct Matrix){o_upper2, nnet->outputSize, 1}};

    float grad_upper1[inputSize], grad_lower1[inputSize];
    struct Interval grad_interval1 = {(struct Matrix){grad_upper1, 1, nnet->inputSize},
                                     (struct Matrix){grad_lower1, 1, nnet->inputSize}};
    float grad_upper2[inputSize], grad_lower2[inputSize];
    struct Interval grad_interval2 = {(struct Matrix){grad_upper2, 1, nnet->inputSize},
    ...
    for(int i=0;i<feature_range_length;i++){
        if(grad->upper_matrix.data[feature_range[i]]<=0 || grad->lower_matrix.data[feature_range[i]]>=0){
            mono = 1;
            #ifdef DEBUG
                printf("mono: %d ",i);  
            #endif    
    ...
    if(mono==1){
        #ifdef DEBUG
            printf("\nmono %d: %f %f\n", split_feature, grad->upper_matrix.data[feature_range[split_feature]], grad->lower_matrix.data[feature_range[split_feature]]);
        #endif
